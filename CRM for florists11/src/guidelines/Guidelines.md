...
└── [Feature]*.tsx       # 📄 Страничные компоненты
```
...
// ✅ Тестируйте user interactions
test('should toggle product status', () => {
  const onToggle = jest.fn();
  render(<ProductItem {...props} onToggle={onToggle} />);
  
  fireEvent.click(screen.getByRole('switch'));
  expect(onToggle).toHaveBeenCalledWith(productId);
});
```
...
export const ProductItem = React.memo(({ product, onToggle }) => {
  // Component logic
});
